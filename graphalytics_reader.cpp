    ...
    while(handle.good()){
        ...
        if ( regex_match(line, matches, pattern) ) { // side effect => populate matches
            ...
            if( key.substr(0, name.length()) != name ){
                cout << "[GraphalyticsReader] Warning, line skipped `" << line << "', graph name does not match what expected: " << name << endl;
                continue;
            }
...
bool GraphalyticsReader::read_edge(uint64_t& out_source, uint64_t& out_destination, double& out_weight){
    return m_impl->read_edge(out_source, out_destination, out_weight);
}
